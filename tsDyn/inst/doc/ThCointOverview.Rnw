...
%\VignetteIndexEntry{Threshold cointegration: overview and implemntation in R}
...
the modelling in the open-source package \tsDyn. It provides hence
...
became a indisensable step in the analysis of non stationary time
...
stationary. The interpretation of this definition is richefull as
it means that the variables have a stable relationship (a long-run
equilibrium), can be represented in an vector error-correction model,
and share a common stochastic trend. 

However, implicit in the definition is the idea that every small deviation
from the long-run equilibrium will lead to error correction mechanisms.
...
stickiness of the prices. Furthermore, it allows to capture asymetries
...
difference-stationnary type. A difference-stationnary (or \emph{integrated})
...
in the software R in package \texttt{urca} (\citealt{PfaffBook2008}). 
...
This is available in R with the ca.jo() function from package \texttt{vars}
...
contest this assumption, such as the presence of menu costs (Lévy
...
by the agents of interventionnary policy only to the error-correction
...
Several other model specifications have been used in the litterature.
...
\footnote{For an analogous case in the structural break litterature, see Gregory
...
Condition (1) corresponds to a case where the stationnarity of the
...
provided the drift in the unit root regime pushs towards the stationary
regime. 
...
received different name in the litterature such as \emph{concentrated
...
such as \texttt{print()}, \texttt{summary(), plot()} and \texttt{toLatext()}
...
0 and 1.%
}. This so called smoothed least sqaure estiamtor has a smaller rate
...
normally distributed and indepednant of the threshold estimatir, this
may not hold in small sample. Hansen (2000), and Seo and Linton (2006),
both suggest methods to take into account the variability of the treshold
...
the authours remark that the variability decrease with the sample
...
my knowledge,tt the validity of that method has not been investigated
...
and speed adjustment prameters can be concentrated out and the estimator
...
the cointegrated relatinship, but becomes intractable with more than
...
such as \texttt{print()}, \texttt{summary()} and \texttt{toLatext()}
...
\end{itemize}
Hence, a test with threhsold cointegration may have as null hypothesis
...
some three regimes, or symetric outer regimes. To my knowledge, only
...
null (alternative) should be choosen when the bound is below (above)
...
a null. In our view, implicetely assumed in that methodology is that
the threshold model will be also stationnary. Whereas the fact that
...
asymptotic p-values. In this procedure, heteroskedasticty can also
...
replications andmerge the result with the old ones. This can be usefull
...
change tests, using techniques as in Hansen (1996). Appropriatness
of this method has been nevertheless discussed by Hansen (2000) as
the the ordering of the variable fot the arranged autoregression may
...
litterature and their use has been sometimes adovacted rather than
...
use of boostrap replications and hence significanty disminishes the
...
Stacking the various parameters into $A_{L}={C_{L},a_{L},B_{L}}$and
similarly for $A_{H}$, the $H_{0}$ of a linear model $A_{H}=A_{L}$is
rejected when $A_{H}\neq A_{L}$. The distribution of the sup-LM test
is found to be the same as in the univariate case as in Hansen (1996)
and hence a p-value simulation as well as a bootstrap replication.
This is then highly computer burdensome as eah bootstrap replication
needs a two-dimensional search. 

This is available in package \tsDyn using the function%
\footnote{it is currently not exported as its object-orientation implementation
was not completed. It can nevertheless still be computed. %
} tsDyn:::HanSeo\_TVECM(). %
...
two major drawback which merit on my opinion more attention. 
...
the cointegrating vector affect the distribution of the test, whch
...
Bec, Ben Salem and Carrasco (\citeyear{BecSalemEtAl2004}) (refered
...
sup-LM and sup-LR are free ofnuisance parameters and provide critical
values. The authors suggest that the extension to a SETAR with non
symetric thresholds should not lead to further complications. 
...
statistics (the sup-Wald, exp Wald and ave Wald) wich are obtain nuisance
...
sup-Wald seelcting the values as in KS depends on nuisance parameters
...
of the boostrap for a three regime model is meant to be easy.
...
based on an alternative model with three regimes, a symetric threshold,
...
More suprisingly, this is still the case with small thresholds, with
...
a much better power than the KS, as the latter is based on a disminishing
threshold effect. 
...
for now in an experimental version and may contain errors. Practitionners
...
can be simply done on testiong whether coefficents from the ECT are
...
$a_{L}$ or $a_{H}$ is dfferent from zero. The sup-Wald test suggested
...
litterature. This is definitely not exhaustive and there exist many
...
interpretation of themodel obtained. A first one concerns the type
of adjustment and the presence of an attractor. A second one concerns
stability of the system and its treactions to exogeneous shocks. 
...
degrees of adjustment, where adjustment pushs back either to an equilibrium
...
faster dynamics than simple AR processes. Despite of this potentally
interesting feature, RD-SETAR models dont seem to have been used much
in the litterature, probably because of the unavailabiliy of tests
...
traditional cointegration as being a better framework to modelize
real world adjustment process with stickiness and asymetries. Indeed,
...
is actually so high that simple aspects such the distribution or the
...
There is up to now no framwork allowing to test the stationarity with
